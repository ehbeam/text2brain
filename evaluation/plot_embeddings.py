...
inputs = ["texts"] # Whether the inputs are from article full texts or titles
batch_size = 64
verbose = True
...
# Averaged embeddings for terms in full texts and titles
X = {}
for inp in inputs:
	m, n_terms = dtm[inp].shape
	X_inp = np.zeros((m, n_emb))
	for i in range(m):
		terms = dtm[inp].columns[dtm[inp].values[i,:] == 1]
		X_inp[i,:] = np.mean(vsm.loc[terms].values, axis=0)
	X[inp] = pd.DataFrame(X_inp, index=dtm[inp].index, columns=range(n_emb)).fillna(0)
...
for inp in inputs:
	...
	fit[inp] = neural_network.Net(n_input=n_emb, n_output=n_structs, n_hid=params[inp]["n_hid"], p_dropout=params[inp]["p_dropout"])
...
for inp in inputs:
	...
	term_map = pd.DataFrame(index=Y.columns, columns=terms)
	...
	for i, term in enumerate(terms):
		...
		pred_probs = fit[inp](term_inputs).float().detach().numpy()
